...
.PHONY: help up backend frontend-install frontend frontend-check test check build deployment-smoke
...
frontend:
	cd frontend && PUBLIC_BACKEND_BASE_URL=$(PUBLIC_BACKEND_BASE_URL) npm run dev
